...
```
/run-harness {goal}                    # team mode (default)
/run-harness {goal} --solo             # single Claude, no team
/run-harness {goal} --timeout 3600
/run-harness status
```

**v2.0: Team mode is now the default.** Claude analyzes the task, designs a team structure autonomously (2–8 agents), and coordinates sub-agents via TeamCreate/TaskCreate/SendMessage. Use `--solo` to opt out.
...
tmux session harness-{run_id}
    ...
    ↓ wait 12s for Claude TUI to fully boot
    ↓ send prompt (+ cursor protocol + team protocol)
    ↓ poll every 5s for .done signal
    ↓ heartbeat stale > 30s → hung → retry
    ↓ dead session → retry
    ↓ exponential backoff [5s, 10s, 20s] × 200
...
```
/run-harness {목표}                    # 팀 모드 (기본)
/run-harness {목표} --solo             # 단독 모드
/run-harness {목표} --timeout 3600
/run-harness status
```

**v2.0: 팀 모드가 기본값.** Claude가 태스크를 분석하여 자율적으로 팀을 구성(2~8명)하고 서브에이전트에 위임한다. `--solo`로 단독 실행 가능.
...
```
/run-harness {目标}                    # 团队模式 (默认)
/run-harness {目标} --solo             # 单独模式
/run-harness {目标} --timeout 3600
/run-harness status
```

**v2.0: 团队模式现在是默认值。** Claude 分析任务后自主设计团队结构（2–8名 Agent），通过 TeamCreate/TaskCreate/SendMessage 协调。用 `--solo` 可选择单独运行。
